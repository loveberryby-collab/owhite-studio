...
export default function ClientProblems({ data }: Props) {
  const d = data || {};
  const title = (d.title as string) || "С какими задачами ко мне приходят";
  const subtitle = (d.subtitle as string) || "Типичные ситуации, в которых я помогаю";
          ...
          {items.map((item, i) => {
            ...
            return (
              <ScrollReveal key={i} delay={i * 0.06}>
                <div className="glass-card group flex items-start gap-4 p-5" data-scroll="" data-scroll-speed={i % 2 === 0 ? 0.02 : 0.05}>
                  ...
                  <p className="text-sm leading-relaxed font-medium text-gray-300">{item.text as string}</p>
                </div>
              </ScrollReveal>
            );
          })}
